        ...
        //
        // ── ADIM 3: GITHUB_CONFIG alanını doldurun ──────────────────────────────────
        //   Aşağıdaki nesnede owner (kullanıcı adınız), repo (depo adı) ve token
        //   (2. Adımda kopyaladığınız anahtar) alanlarını doldurun. Dosyayı kaydedip
        //   yeniden yükleyin. GITHUB_CONFIG dolu olduğu an bulut senkronizasyonu
        //   OTOMATİK devreye girer; doldurmazsanız uygulama eskisi gibi (sadece bu
        //   tarayıcıda/localStorage) çalışmaya devam eder, hiçbir şey bozulmaz.
        //
        // ── ⚠ GÜVENLİK UYARISI (ÖNEMLİ, MUTLAKA OKUYUN) ─────────────────────────────
        //   Bu token, web sayfasının JavaScript koduna gömülü olduğundan, sayfayı
        //   ziyaret eden HERKES "Sayfa Kaynağını Görüntüle" ile bu anahtarı görebilir.
        //   Bu yüzden:
        //   • Token'ı MUTLAKA sadece 1. Adımdaki tek/küçük depoya, sadece "Contents:
        //     Read and write" izniyle sınırlandırın (2. Adım f/g).
        //   • Bu token'ı ASLA başka bir depoda/projede kullanmayın.
        ...
        const GITHUB_CONFIG = {
            ...
            token: "BURAYA_GITHUB_TOKENINIZI_YAZIN", // 2. Adımda ürettiğiniz fine-grained token
        ...
        const CLOUD_SYNC_ENABLED = !!(
            GITHUB_CONFIG.token && GITHUB_CONFIG.token.indexOf('BURAYA_') === -1 &&
        ...
        let cloudDataSha = null;          // data/pano-config.json dosyasının güncel sürüm (sha) bilgisi
        ...
        // Depodaki bir dosyayı okur. Dosya yoksa null döner (ör. ilk kurulumda).
        ...
        // görselin raw.githubusercontent.com bağlantısı (küçük bir metin) saklanır. Bulut etkin
        // değilse veya yükleme başarısız olursa null döner ve çağıran taraf eskisi gibi
        // base64'ü (sadece bu cihazda) kullanmaya devam eder.
        function uploadImageFileToCloud(file, folderHint) {
            if (!CLOUD_SYNC_ENABLED || !file) return Promise.resolve(null);
        ...
        function cloudSyncPullOnce() {
            ...
            githubGetFile(GITHUB_CONFIG.dataPath).then(result => {
                if (!result) {
                    // Depoda henüz hiç kayıt yok (ilk kurulum) — mevcut yerel veriyi buluta yükle.
                    cloudPushNow();
                    return;
                }
                cloudDataSha = result.sha;
                let cloudData;
        ...
        // aralıklarla (varsayılan 25 sn) depo dosyası kontrol edilir (yoklama/polling). Başka bir
        // cihazdan değişiklik geldiyse ve Yönetim Paneli o an açık DEĞİLSE, panoyu otomatik
        // tazeler. Panel açıksa aktif düzenlemeyi bozmamak için dokunulmaz; localStorage zaten
        ...
        function cloudPushNow() {
            ...
            githubPutFile(GITHUB_CONFIG.dataPath, base64Content, 'Pano verisi güncellendi', cloudDataSha).then(result => {
                if (result && result.content) cloudDataSha = result.content.sha;
            }).catch(err => {
                if (err && err.status === 409) {
                    // Başka bir cihaz aynı anda yazdı (sürüm/sha uyuşmazlığı) — güncel sürümü alıp bir kez daha dene.
                    githubGetFile(GITHUB_CONFIG.dataPath).then(result => {
                        cloudDataSha = result ? result.sha : null;
                        return githubPutFile(GITHUB_CONFIG.dataPath, base64Content, 'Pano verisi güncellendi (tekrar deneme)', cloudDataSha);
                    }).then(result => {
                        if (result && result.content) cloudDataSha = result.content.sha;
                    }).catch(err2 => {
