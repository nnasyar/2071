        ...
        // cihaz/tarayıcıdan açıldığında veriler görünmez. Aşağıdaki FIREBASE_CONFIG
        // alanını kendi (ücretsiz) Firebase projenizin bilgileriyle doldurursanız,
        // tüm pano verisi (fotoğraflar dahil) Firestore'a da yazılır ve panoyu hangi
        // cihazdan/tarayıcıdan açarsanız açın aynı güncel veriyi görürsünüz.
        //
        // KURULUM:
        // 1) https://console.firebase.google.com adresinden ücretsiz bir proje oluşturun.
        // 2) Sol menüden "Build > Firestore Database" i açıp "Create database" ile
        //    Firestore'u etkinleştirin (test modunda başlayabilirsiniz).
        // 3) Proje Ayarları (⚙) > "Genel" sekmesinde "Web uygulaması ekle" (</>) ile
        //    bir web uygulaması tanımlayın; size verilen firebaseConfig nesnesini
        //    aynen aşağıya yapıştırın.
        ...
        if (CLOUD_SYNC_ENABLED && typeof firebase !== 'undefined') {
            try {
                firebase.initializeApp(FIREBASE_CONFIG);
                cloudDb = firebase.firestore();
                cloudDocRef = cloudDb.collection('okulPano').doc('config');
            } catch (e) {
                console.warn('Firebase başlatılamadı, sadece yerel (bu tarayıcı) modda çalışılacak:', e);
                cloudDb = null;
                cloudDocRef = null;
            }
        }
        ...
        // (hızlı, çevrimdışı da çalışır). Ardından buluttaki kayıt daha güncelse
        // (örn. başka bir cihazdan yapılan bir değişiklik varsa) arka planda indirilip
        // sayfa otomatik tazelenir; böylece her cihaz aynı güncel veriyi gösterir.
        function cloudSyncPullOnce() {
            if (!cloudDocRef) return;
            cloudDocRef.get().then(doc => {
                if (!doc.exists) {
                    // Buluta henüz hiç kayıt atılmamış (ilk kurulum) — mevcut yerel veriyi buluta yükle.
                    cloudPushNow();
                    return;
                }
                const cloudData = doc.data() && doc.data().appConfig;
                if (!cloudData) return;
        ...
        // Buluta gerçek zamanlı bir değişiklik gelirse (başka bir cihazdan kayıt yapıldıysa)
        // ve Yönetim Paneli o an açık DEĞİLSE, panoyu otomatik tazeler. Panel açıksa
        // aktif düzenlemeyi bozmamak için dokunulmaz; localStorage zaten güncellenmiştir,
        // panel kapatılınca doğal akışta yeni veriyle devam eder.
        function cloudSyncStartListening() {
            ...
            cloudDocRef.onSnapshot(doc => {
                ...
                console.warn('Bulut dinleyicisi hata verdi:', err);
            });
        }
        ...
        // her karede değil, kullanıcı durduktan ~900ms sonra tek seferde yazılır.
        function cloudPushDebounced() {
            if (!cloudDocRef) return;
            clearTimeout(cloudWriteTimer);
            cloudWriteTimer = setTimeout(cloudPushNow, 900);
        }

        function cloudPushNow() {
            if (!cloudDocRef) return;
            const json = JSON.stringify(appConfig);
            lastSyncedConfigJSON = json;
            cloudDocRef.set({
                appConfig: appConfig,
        ...
        function moduleSettingsAddNew() {
            ...
            if (!appConfig.moduleSettings) appConfig.moduleSettings = {};
            appConfig.moduleSettings[newId] = { title: 'Yeni Modül Başlığı', content: 'Bu alana metin yazabilirsiniz.', color: '', bgType: '', bgColor1: '', bgColor2: '', titleBgType: '', titleBgColor1: '', titleBgColor2: '', titleColor: '', font: '', size: 'normal', active: true, titleActive: true, cellEffect: 'none', cellEffectColor: '#00b4d8', cellEffectIntensity: 100 };
            _rebuildModuleDefs();
        ...
        window.onload = function() {
            ...
            // FIREBASE_CONFIG doldurulmuşsa, arka planda buluttan daha güncel bir kayıt olup
        ...
        function renderPanoData() {
            ...
            (appConfig.achievementCategories || []).forEach(cat => cycleAchievementCategory(cat.id));
            cycleMediaSlides();
            renderActiveDuties();
        ...
        function renderAchievementsCard() {
            ...
            if (!container) return;
            const cats = appConfig.achievementCategories || [];
        ...
        function startCyclingModuleIntervals() {
            ...
            (appConfig.achievementCategories || []).forEach(cat => {
...
`;
        ...
        // Ayırıcı türü "Simge" seçilmediyse simge alanını gizle
        ...
        function handleAchievementFileSelect(catId, inputEl) {
            ...
            const reader = new FileReader();
            reader.onload = function (e) {
                achievementPendingFile[catId] = e.target.result;
